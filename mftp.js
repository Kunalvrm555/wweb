...
const sendmedia = async (string_base64, caption_text) => {
    const media = new MessageMedia('application/pdf', string_base64, caption_text.substring(caption_text.length - 16));
    //   await client.sendMessage(chatId, media,{caption: 'this is my caption'}, MessageTypes.DOCUMENT);
    const chats = await client.getChats();
    //   console.log(chats);
    chats.forEach(async (chat) => {
        if (chat.isGroup) {
